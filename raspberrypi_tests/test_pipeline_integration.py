...
def capture_published():
    """Patch _mqtt_client.publish and return a list that collects payloads."""
    captured = []
...
def test_index_conversion_zero_to_one():
    """CV returns 0-indexed; the pipeline loop must convert before publish."""
    reset_state()
    captured = capture_published()
...
def test_index_conversion_boundary():
    """Bottom-right corner: CV (14,14) must publish as (15,15)."""
    reset_state()
    captured = capture_published()
